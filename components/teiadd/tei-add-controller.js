...
eRegistry.controller('TEIAddController', 
    function($scope, 
            $rootScope,
            $translate,
            $modalInstance, 
    ...
    $scope.search = function(mode){ 
        ...
        if( $scope.addingTeiAssociate ){            
            if(!$scope.selectedTrackedEntity){
                ...
                $scope.programUrl = $scope.programUrl ? $scope.programUrl + '&trackedEntityType=' + $scope.selectedTrackedEntity.id : 'trackedEntityType=' + $scope.selectedTrackedEntity.id;
            }
        }
    ...
    $scope.fetchTei = function(){
        ...
        TEIService.search($scope.selectedOrgUnit.id, 
            ...
            if(!$scope.sortColumn.id){                                      
                $scope.sortGrid({id: 'created', displayName: $translate.instant('registration_date'), valueType: 'date', displayInListNoProgram: false, showFilter: false, show: false});
            }
    ...
    $scope.generateGridColumns = function(attributes){
        ...
        columns.push({id: 'orgUnitName', displayName: 'Organisation unit', type: 'TEXT', displayInListNoProgram: false});
        columns.push({id: 'created', displayName: 'Registration date', type: 'TEXT', displayInListNoProgram: false});
        
        //generate grid column for the selected program/attributes
        angular.forEach(columns, function(column){
            if(column.id === 'orgUnitName' && $scope.selectedOuMode.displayName !== 'SELECTED'){
                column.show = true;
            }
...
.controller('TEIRegistrationController', 
    ...
    $scope.registerEntity = function(){
        ...
        $scope.tei = {trackedEntityType: selectedTrackedEntity, orgUnit: $scope.selectedOrgUnit.id};
